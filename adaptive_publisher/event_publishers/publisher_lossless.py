...
class EventPublisher():
    ...
    def generate_event_from_frame(self, frame, frame_index, trace_id=None):
        ...
        # PNG compression: 1 fastest (still lossless), 9 smallest (slow)
        png_level = int(os.getenv("PNG_COMPRESSION_LEVEL", "1"))
        ...
        try:
            ...
            # -------------------------
                ...
                check_print = (frame.shape, frame.dtype, frame.flags["C_CONTIGUOUS"], frame.nbytes)
                ...
                if span:
                    ...
                    span.set_tag("payload.bytes", payload_bytes)
                    span.set_tag("png.check_print", str(check_print))
            ...
            # -------------------------
                ...
                enc0 = time.time()
                img_rgb = frame[:, :, ::-1]  # BGR -> RGB
                jxl_bytes = imagecodecs.jpegxl_encode(img_rgb, level=0, lossless=True, effort=jxl_effort)
